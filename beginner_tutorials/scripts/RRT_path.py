...
while (goal_reached==False):
    ...
    #for i in range(len(nodes_x)):
    #    if (sqrt(pow(( nodes_x[i]-goalx), 2) + pow((nodes_y[i]-goaly), 2)))<0.5:
    #        c= 5005
    #        print('goal reached')
    #print(parent_index)
    c=c+1  
...
path.append(0)    
#print(path)
#print(sqrt(pow(( nodes_x[path[0]]-goalx), 2) + pow((nodes_y[path[0]]-goaly), 2)))
#print(nodes_x[path[0]])
#print(nodes_y[path[0]])

#while (sqrt(pow(( nodes_x[path[0]]-goalx), 2) + pow((nodes_y[path[0]]-goaly), 2))<0.1 ):
#    for j in range(len(path)-1):
#        plt.plot([nodes_x[path[j]],nodes_x[path[j+1]]], [nodes_y[path[j]],nodes_y[path[j+1]]], "k.-", markersize = 3, linewidth = 0.3)    
for i in range(len(path)-1):
   plt.plot([nodes_x[path[i]],nodes_x[path[i+1]]], [nodes_y[path[i]],nodes_y[path[i+1]]], "k.-", markersize = 3, linewidth = 0.3)
#plt.plot([nodes_x[path[len(path)-1]],nodes_x[parent_index[len(parent_index)]]], [nodes_y[path[len(path)-1]],nodes_y[parent_index[len(parent_index)]]], "k.-", markersize = 3, linewidth = 0.3) 
...
pub_pathx = rospy.Publisher('final_pathx', Complex)
pub_pathy = rospy.Publisher('final_pathy', Complex)

rate = rospy.Rate(10) 
while not rospy.is_shutdown():
    pathx = Complex()
    pathy = Complex()
    pathx.array=final_pathx
    pathy.array=final_pathy
    pub_pathx.publish(pathx)
    pub_pathy.publish(pathy)
    rate.sleep()
    #print(final_pathx)
